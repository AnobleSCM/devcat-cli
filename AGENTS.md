...
## PR workflow — required; AI review gate decommissioned

The AI review gate was removed fleet-wide on 2026-07-24. There is no
review-self-check requirement, clean-signal prerequisite, hosted reviewer,
required review status check, signed receipt loop, protected-path reviewer
quorum, or local review-gate hook.

For every code change: branch from fresh origin/main, open a draft PR, review
your own diff, run the verification the change deserves, and merge the PR.
Never push directly to main unless Andrew explicitly says "commit directly" or
"skip the PR" for that task. For high-blast-radius or protected-path work, get
a panel second opinion before merging; it is advisory, not a merge gate.

Do not re-add required review checks, restart the Atlas review runner,
re-register a hook, rebuild the former gate, or invent a local substitute.
Current workflow detail: ~/.agents/reference/pr-workflow.md.
